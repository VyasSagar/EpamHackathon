...
public class ExcelExternalWorkbook extends ExcelConnectionManager {
	...
	public synchronized static boolean clearSheet(String filename, String sheetName) {
		String query = "DELETE from `" + sheetName + "`";
		
		try {
			getExternalWorkbookConnection(filename).executeUpdate(query);
			return true;
		} catch (FilloException e) {
			e.printStackTrace();
			return false;
	...
	public synchronized static List<String> getColumnsAsList(String filePath, String sheetName,String colName){
		List<String> value = new ArrayList<String>();
		Recordset recordset;
		String strQuery = "Select * from `" + sheetName + "`";
		try {
			recordset = getExternalWorkbookConnection(filePath).executeQuery(strQuery);
			if (!recordset.next())
				return null;
			else{
				do{
